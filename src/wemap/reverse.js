...
export default class Reverse {
    ...
    getReverseData(e) {
        return new Promise((res, rej) => {
            makeRequest({
                url: `${config.reverse}?point.lat=${e.lngLat.lat}&point.lon=${e.lngLat.lng}`,
                method: 'GET',
                key: this.key
            }, (err, data) => {
                if (err) rej(err)
    ...
    getReversePolygonData(e) {
        return new Promise((res, rej) => {
            makeRequest({
                url: `${config.reverse}?point.lat=${e.lngLat.lat}&point.lon=${e.lngLat.lng}&layers=locality`,
                method: 'GET',
                key: this.key
            }, (err, data) => {
                if(err) rej(err)
    ...
    getReverseStreetData(e){
        return new Promise((res, rej) => {
            makeRequest({
                url: `${config.reverse}?point.lat=${e.lngLat.lat}&point.lon=${e.lngLat.lng}&layers=street`,
                method: 'GET',
                key: this.key
            }, (err, data) => {
                if (err) rej(err)
